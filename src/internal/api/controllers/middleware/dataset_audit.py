...
class DatasetControllerAudit():
    ...
    def dataset_service_store_dataset(self, user, body: TemplatebackendDataset): #TODO what is in user?
        body_serialized = f"id: {body.id or ''}, dataset_name: {body.dataset_name or ''}"
    ...
    def dataset_service_list_datasets(self, user, name: str, offset: int=None, limit: int=None):
        try:
            response : TemplatebackendListDatasetsReply =  self.next.dataset_service_list_datasets(user, name,offset,limit)
