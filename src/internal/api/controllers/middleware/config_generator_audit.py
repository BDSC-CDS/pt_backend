...
class ConfigGeneratorServiceControllerAudit():
    def __init__(self, next: ConfigGeneratorServiceController,auditLogService: AuditLogService):
        self.next = next
        self.auditLogService = auditLogService
        implements_interface(ConfigGeneratorServiceController, ConfigGeneratorServiceControllerAudit)

    def configuration_service_get_configs(self, user):

        try:
            response : TemplatebackendGetConfigsReply =  self.next.configuration_service_get_configs(user)
    ...
    def configuration_service_create_config(self, user,body: TemplatebackendConfig):
        ...
        try:
            response : TemplatebackendCreateConfigReply =  self.next.configuration_service_create_config(user,body)
    ...
    def configuration_service_delete_config(self, user, id:int):
        try:
            response : TemplatebackendDeleteConfigReply =  self.next.configuration_service_delete_config(user, id)
    ...
    def configuration_service_export_config(self,user,id:int):
        try:
            response : TemplatebackendExportConfigReply =  self.next.configuration_service_export_config(user, id)
