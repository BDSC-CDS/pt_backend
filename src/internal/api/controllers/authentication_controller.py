...
class AuthenticationServiceController():
    ...
    def authentication_service_authenticate(self, user, body: TemplatebackendCredentials):
        try:
            ...
            self.audit_log_service.log_event(AuditLog(service="AuthenticationServiceController", action="authenticated user",body=body.username))
            return TemplatebackendAuthenticationReply(TemplatebackendAuthenticationResult(token)), 200
        except Exception as e:
            self.audit_log_service.log_event(AuditLog(service="AuthenticationServiceController", action="Error authenticating",body=body.username, error=True))
            raise e
