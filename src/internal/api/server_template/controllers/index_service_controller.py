"""NOTE: Autogenerated. Do not edit the manually."""

import connexion
from typing import Dict
from typing import Tuple
from typing import Union
from inspect import getmembers, isfunction, ismethod


from server_template.models.index_service_create_hello_request import IndexServiceCreateHelloRequest
from server_template.models.rpc_status import RpcStatus
from server_template.models.templatebackend_create_hello_reply import TemplatebackendCreateHelloReply
from server_template.models.templatebackend_get_hello_reply import TemplatebackendGetHelloReply
from server_template import util


#from src.internal.api.controllers import index_service_controller
#controller_functions =  [func_tupple[0] for func_tupple in getmembers(index_service_controller, isfunction)]
#needed_functions = ["index_service_create_hello", "index_service_get_hello"]
#for op in needed_functions:
#    if op not in controller_functions:
#        raise NotImplementedError("operation " + op + " is not implemented by src.internal.api.controllers.index_service_controller")

class IndexServiceController:
    def __init__(self, controller):
        #controller_functions =  [func_tupple[0] for func_tupple in getmembers(controller, ismethod)]
        #needed_functions = ["index_service_create_hello", "index_service_get_hello"]
        #for op in needed_functions:
        #    if op not in controller_functions:
        #        raise NotImplementedError("operation " + op + " is not implemented by provided controller")
        util.implements_interface(IndexServiceController, controller)

        self.controller=controller


    def index_service_create_hello(self, user, identifier: int, body: IndexServiceCreateHelloRequest):
        """Get a hello

        This endpoint returns a hello

        :param identifier: 
        :type identifier: int
        :param body: 
        :type body: dict | bytes

        :rtype: Union[TemplatebackendCreateHelloReply, Tuple[TemplatebackendCreateHelloReply, int], Tuple[TemplatebackendCreateHelloReply, int, Dict[str, str]]
        """
        if connexion.request.is_json:
            body = IndexServiceCreateHelloRequest.from_dict(connexion.request.get_json())

        return self.controller.index_service_create_hello(user, identifier, body)


    def index_service_get_hello(self, user):
        """Get a hello

        This endpoint returns a hello


        :rtype: Union[TemplatebackendGetHelloReply, Tuple[TemplatebackendGetHelloReply, int], Tuple[TemplatebackendGetHelloReply, int, Dict[str, str]]
        """

        return self.controller.index_service_get_hello(user, )
