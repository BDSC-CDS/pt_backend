...
class TemplatebackendStoreDatasetRequest(Model):
    ...
    def dataset(self, dataset: str):
        ...
        :type dataset: str
        """
        if dataset is not None and not re.search(r'^(?:[A-Za-z0-9+\/]{4})*(?:[A-Za-z0-9+\/]{2}==|[A-Za-z0-9+\/]{3}=)?$', dataset):
            raise ValueError("Invalid value for `dataset`, must be a follow pattern or equal to `/^(?:[A-Za-z0-9+\/]{4})*(?:[A-Za-z0-9+\/]{2}==|[A-Za-z0-9+\/]{3}=)?$/`")
