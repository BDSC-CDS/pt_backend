...
class TemplatebackendQuestionnaireQuestionAnswer(BaseModel):
    """
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in rule_prefills (list)
        _items = []
        if self.rule_prefills:
            for _item in self.rule_prefills:
                if _item:
                    _items.append(_item.to_dict())
            _dict['rulePrefills'] = _items
        return _dict
